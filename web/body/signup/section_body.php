<div class="home_container">
    <div id="signupBanner">
        <p class="signupBanner-head">ngulikin</p>
		<p class="signupBanner-body">registrasi di ngulikin</p>
		<div class="signupBanner-footer">
		    <div>Sudah punya akun Ngulikin masuk</div>
		    <div>disini</div>
		</div>
    </div>
	<section class="container">
		<div class="grid-signup-body">
			<div class="grid-signup-bodyhead">
			    registrasi
			</div>
			<div class="grid-signup-bodyfooter">
			    Dengan memiliki akun Ngulikin.com, Anda dapat melakukan transaksi. Jangan khawatir data anda akan aman bersama kami.
			</div>
			<div class="signupBodySub">
        		<input type="text" id="usernameSignUp" class="inputSignup" placeholder="Username"/>
        		<i class="fa fa-user"></i>
        		<span>*Wajib diisi</span>
        	</div>
			<div class="signupBodySub">
        		<input type="text" id="nameSignUp" class="inputSignup" placeholder="Nama Lengkap"/>
        		<i class="fa fa-user"></i>
        		<span>*Wajib diisi</span>
        	</div>
        	<div class="signupBodySub">
        		<input type="text" id="emailSignUp" class="inputSignup" placeholder="Email"/>
        		<i class="fa fa-envelope-o"></i>
        		<span>*Wajib diisi</span>
        	</div>
        	<div class="signupBodySub">
        		<input type="text" id="nohpSignUp" class="inputSignup" placeholder="Nomor Telepon"/>
        		<i class="fa fa-mobile-phone"></i>
        		<span>*Wajib diisi</span>
        	</div>
        	<div class="signupBodySub">
        		<input type="password" id="passSignUp" class="inputSignup" placeholder="Password"/>
        		<i class="fa fa-lock"></i>
        		<span>*Wajib diisi</span>
        	</div>
        	<div class="signupBodySub">
        		<div class="signupBodySubDate">
        		    <span>Tanggal Lahir</span>
        			<input type="text" id="dateSignUp" class="inputSignup" data-inline="false" data-role="date"/>
        			<span>*Wajib diisi sesuai KTP</span>
        	    </div>
        		<div class="signupBodySubSex">
        		    <div>
        		       <span id="sextextSignUp">Jenis Kelamin</span> 
        		    </div>
        			<div class="select">
        			    <select id="sexSignup">
            			    <option value="male">Laki-laki</option>
            			    <option value="female">Wanita</option>
            			</select>
        			</div>
        		</div>
        	</div>
        	<div class="signupBodySub signupButton">
        	    <input type="hidden" id="socmedSignUp" value="ngulikin"/>
        	    <input type="hidden" id="idsocmedSignUp" value=""/>
        	    <div id="signup_message" class="green"></div>
        		<button type="button" id="buttonSignup">DAFTAR</button> 
        	</div>
        	<div class="signupBodySub">
        		<span class="signupConfirmText confirm">
        		    Dengan menekan tombol Daftar, saya mengkonfirmasi telah menyetujui Syarat dan Ketentuan, serta Kebijakan Privasi Ngulikin.com
        		</span>
			</div>
		</div>
	</section>
</div>